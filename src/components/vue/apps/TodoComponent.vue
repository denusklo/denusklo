  ...
  setup() {
    ...
    const filteredTodos = computed(() => {
      if (currentFilter.value === "all") {
        ...
        return todoStore.getAllTodos.filter(todo => !todo.completed);
      } else {
        return todoStore.getAllTodos.filter(todo => todo.completed);
      }
    });
    ...
    const toggleComplete = (index) => {
      ...
      todoStore.toggleTodoComplete(index);
    };

    const editTodo = (index) => {
      editIndex.value = index;
      editText.value = todoStore.getAllTodos[index].text;
      isEditing.value = true;
    };

    const saveEdit = () => {
      if (editText.value.trim() !== "") {
        todoStore.updateTodo(editIndex.value, editText.value);
        cancelEdit();
      }
    };
    ...
    const completedCount = computed(() => {
      return todoStore.getAllTodos.filter(todo => todo.completed).length;
    });

    return {
      AddTodo,
      mytodo,
      todos: computed(() => todoStore.getAllTodos),
      isEmpty: computed(() => todoStore.todoEmpty),
